...
### 3-83. 진행 안내 문구를 하단 안내 띠로 교체 — 지도 위 흰 글씨라 안 읽히던 문제 (2026-08-05, 사용자 스크린샷 2장)

**증상**: "정보를 전달할 장소를 선택하세요" 같은 진행 안내가 **지도 한가운데에 배경 없이 흰색
17pt**로 떠서, 흑백 지도 그림과 섞여 거의 읽히지 않았다(사용자: "너무 작고 폰트도 하얀색이라 안 보여").

**원인**: `Instruction`이 앵커 `(0.32,0.31)~(0.64,0.40)`으로 **화면 한가운데 지도 위**에 있고,
`InstructionText`는 `SUIT-Light 17` 흰색에 **배경 판이 전혀 없었다**. 지도 배경이 밝은 회색~흰색
톤이라 흰 글씨가 그대로 묻힌다.

**수정** — 화면 하단 전체 폭 안내 띠로 옮겼다(나레이션 가이드의 어두운 띠 + 구분선 구성 참고):

| | 이전 | 이후 |
|---|---|---|
| 위치 | 화면 중앙 지도 위 | 하단 전체 폭, 손패 카드 바로 위 |
| 배경 | **없음** | 어두운 판 `#171311` 알파 0.9 |
| 폰트 | SUIT-Light **17** | SUIT-SemiBold **28** |
| 색 | 순백 `#FFFFFF` | `#F4EFE7` (배경이 어두워 대비 확보) |
| 정렬 | TopLeft | Center |
| 장식 | 없음 | 상단 강조선 3px `#B2332E` |

**띠를 어디에 끼울지도 실측으로 정했다.** 손패 카드(460×230, 바닥 앵커 y −10)의 위쪽 끝이
캔버스 중앙원점 기준 **−435**, 접선 카드(뒷골목)의 아래쪽 끝이 약 **−343**이라 그 사이 92px가
비어 있다. 처음에 잡은 높이 96은 뒷골목 카드를 23px 침범해서 **84로 줄이고 중심 −389**로 옮겨
`−431 ~ −347`에 정확히 끼웠다(위아래 4px씩 여유).

**검증**(Play Mode, Zone1): 안내 4문구 전부 **1줄**에 들어감(가장 긴 것 1103/1320px),
그려진 글리프 44/44. 화면상 띠는 `x 260~1660 / y 109~193`(1920×1080)이고 **손패 카드 4장과
겹침 0건, 접선 카드와도 겹침 없음**. Console Error/Warning 0. 씬 무수정.

**수정한 파일**: `PlayHudCanvas_New.prefab`(Instruction 재구성 - Plate/TopLine 추가, 텍스트 스타일·위치).
...
### 3-85. 안내 문구 3중 위젯을 하단 띠 하나로 통합 + 지시 문구 삭제 + NPC 말풍선 재작업 (2026-08-05, 사용자 판단)

사용자 질문("이런 안내문구들은 이제 없애는 게 나을라나?")에서 출발. 스크린샷에 보이던 띠는
**3-83에서 만든 하단 띠가 아니라 별개의 옛 위젯**이었고, 조사해 보니 안내 계열이 셋으로 갈라져 있었다.

| 위젯 | 옛 위치 | 표시 조건 |
|---|---|---|
| `Instruction` | 하단 띠(3-83) | 카드 선택 시 |
| `FeedbackBanner` | **지도 한가운데**, 앵커 `x 0.25~0.99`(중심이 화면 62%라 우측으로 치우침), fs 14 | 일시 알림 |
| `NoSelectionHint` | BorrowedBottomPanel 안 | 카드 미선택 시 |

**1) 문구를 "지시"와 "보고"로 갈라 지시만 삭제했다.** 기준: *플레이어가 화면만 봐도 아는 것*은
빼고, *규칙이 뒤에서 움직인 사실*만 남긴다. 지시문은 반복 플레이에서 잔소리가 되지만 보고문은
100턴째에도 필요하다.

- 삭제: `"먼저 카드를 선택하세요."`(2곳, TargetingController) / `"전달할 정보를 선택하세요."`(NoSelectionHint)
  / `"결과를 확인하세요."`(CardPlayedEvent) / `"정보를 전달할 장소·대상을 선택하세요."`(AwaitingTarget)
- 유지: `"보유 정보가 부족해 새로운 정보 N개를 획득했습니다."`(손패가 늘어난 이유는 화면에 안 나온다)
  / `"지금 단계에서는 다른 행동을 할 수 없습니다."` / 카드 타입 불일치·접근 제한 등 **거부 사유** 전부
  / 접선 태그 안내(AwaitingConfirm — 태그가 새 UI라 위치를 모르면 진행이 막힘)

**2) 위젯 3개를 띠 하나로 합쳤다.** `FeedbackBanner`를 `Instruction` 안으로 옮겨 `Notice`로 개명,
자체 판(Image)은 제거하고 새 `Bg`(판 + 상단 강조선)를 지속 안내와 공유한다. `NoSelectionHint`는 폐기.
`HudPresenter`에 `barInstruction`/`noticeShowing` 두 상태와 `RefreshBarVisibility()`를 넣어
**알림이 뜬 동안에는 지속 안내를 감춰** 같은 자리에 두 문장이 겹치지 않게 했고, 둘 다 없으면 판까지 꺼서
빈 띠가 지도 위에 남지 않는다. `feedbackBannerRect`는 띠 전체를 가리키게 재배선해서 Profile/Log 패널이
열릴 때 폭을 좁히는 기존 로직(`SetHudPanelState`)이 그대로 적용되도록 앵커 기반으로 바꿨다.

**3) 띠 위치를 다시 잡았다 — 3-83 수치가 틀렸다.** 3-83은 손패 카드를 **접힌 상태**(상단 −435)만
보고 `−431~−347`을 잡았는데, **선택돼 펼쳐진 카드는 −312까지 123px 더 솟는다**. 즉 이 띠가 뜨는 바로 그
상황에서 방금 고른 카드의 제목 줄을 가리고 있었다(실측·스크린샷 확인). 펼쳐진 카드 위로 올리고
(`−306~−222`, 여유 6px), 지도 왼쪽 아래 접선 카드(`x −761~−641`)를 절대 가리지 않게 폭을
1400 → **1240**(`x −620~620`)으로 줄였다.

**4) 띠가 클릭을 먹던 문제도 같이 고쳤다.** 판/글자의 `raycastTarget`이 켜져 있어 띠 아래의 지도와
장소 카드 클릭이 막히고 있었다. 전부 끄고 레거시 전달 버튼만 되살렸다(비활성 오브젝트에서는
`GetComponentInParent`가 null을 돌려줘 "버튼 제외" 가드가 안 걸렸던 것 — **비활성 대상에 이 API를
쓸 때 반복될 함정**). 검증: 띠 한가운데 `EventSystem.RaycastAll` → **0건**.

**5) NPC 말풍선 전면 재작업.** 증상은 지도 위에 뜨는 거대한 흰 글씨. 원인이 세 개였다.

| | 이전 | 이후 |
|---|---|---|
| 위치 | 캐릭터 머리 위 **+3.30**(키의 3.3배 위 — 누가 말하는지 안 보임) | 꼬리 끝이 머리 바로 위(**+0.66**, 여유 7px) |
| 크기 | 월드 고정 fs 4 → 카메라 줌에 그대로 끌려다님 | 기준 줌(ortho 5) 대비 비율로 런타임 보정 → **어느 스테이지든 화면상 동일** |
| 배경 | `Background`가 글자보다 **0.65 위로 어긋나** 있고 스케일이 `(6.4, 1.5)` 비균일(=말풍선이 찌그러짐) | 실제 아트 `NPC 대화 UI`(236×98, 꼬리 아래) 균일 스케일, 꼬리 끝 기준 정렬 |
| 글자색 | 순백 | 잉크색 `#241E1A`(말풍선이 밝은 회색이라) |

줌 문제가 핵심이다 — `orthographicSize`가 Zone1은 5, Metropolis는 14라 **2.8배** 차이가 나서,
고정 크기로는 같은 글자가 Zone1에서 62px / Metropolis에서 22px가 된다(실측). 그래서 프리팹 수치는
`DialogueReferenceOrthoSize = 5`에서 한 번만 맞추고, `FitDialogueToCamera()`가 표시 직전에
`ortho / 기준값`만큼 되곱한다(루트의 캐릭터 보정 스케일 `lossyScale`은 나눠서 상쇄).
**앵커를 꼬리 끝에 둔 것이 요점** — 그래야 스케일이 커질 때 말풍선이 위로만 자라고 꼬리는 머리에 붙어 있는다.
좌표는 스프라이트 알파를 직접 재서 뽑았다(꼬리 끝 = 중심에서 −45/98, 상자 중심 = +9.5/98).

**검증**(Play Mode, Zone1, 1920×1080):
- 띠 4상태 전부 정상 — 카드 미선택 `안 뜸` / 카드 없이 지도 클릭 `안 뜸`(예전엔 "먼저 카드를…") /
  AwaitingTarget `안 뜸` / AwaitingConfirm `접선 태그 안내` / 전달형을 장소에 클릭 `거부 사유(#F27366)` ✅
- 알림이 뜨면 지속 안내가 자동으로 숨고, 둘 다 없으면 판까지 꺼짐 ✅
- 띠 `x −620~620 / y −306~−222`, **손패 카드 4장 전부 겹침 0**(가장 높이 솟은 카드 −312, 여유 6px) ✅
- 접선 카드(뒷골목)와 겹침 없음 ✅ / 띠 중앙 레이캐스트 0건 ✅
- 안내 문구 글리프 **44/44**, 1줄, 글자폭 1103 / 상자폭 1160 ✅
- 말풍선: 화면상 **236×98px**(줌 무관), 글리프 **22/22**, 2줄, 글자가 말풍선 상자 안에 전부 들어감 ✅
- Console Error/Warning 0, 씬 무수정 ✅

**알려진 절충**: 지도가 빽빽해서 **가로 띠가 무엇과도 안 겹치는 빈 밴드가 없다**. Zone1 기준
`여관` 장소 카드(`x 59~157, y −326~−167`)를 띠가 가로지른다. 클릭은 통과하므로(4번) 기능 문제는
없고, 카드 이름표는 띠 아래로 보인다. "펼쳐진 카드 제목을 가리는 것"보다 이쪽이 낫다고 판단했으나,
위치를 다시 옮기고 싶으면 `Instruction.anchoredPosition.y`(현재 −264) 하나만 바꾸면 된다.

**수정한 파일**: `HudPresenter.cs`, `HudView.cs`, `TargetingController.cs`, `NpcActorView.cs`,
`PlayHudCanvas_New.prefab`, `NpcActorView.prefab`.
...
- ~~헤더의 스테이지 이름이 태그 아트를 넘친다~~ → 3-90에서 처리.
...
### 3-90. 헤더 스테이지명 태그를 이름 길이에 맞춰 늘어나게 (2026-08-05, 사용자 스크린샷)

**증상**: 이름이 길면 태그 아트 밖으로 넘쳐 두 줄로 접히고 잘렸다("귀족가 · 영 / 주 저택 인근").

**원인**: `HeaderArea/StageCard`가 **고정 폭 147**이고 이름 상자는 120인데, 실제로 들어가는 건
`StageData.regionName`이라 스테이지마다 7~13자로 크게 다르다(`북문(외곽)` / `상업지구 · 시장가` /
`귀족가 · 영주 저택 인근` / `클레르폰 대도시 전역`). 상자가 좁으니 TMP가 줄바꿈해서 태그 밖으로 흘렀다.

참고로 이 자리를 채우는 건 `HudPresenter.RefreshHeader()`가 아니라 **`StageRegionNameAdapter`**다
(`HudView.stageNameText`는 이 화면에서 비어 있고, `StageData.stageName`도 비어 있어 어댑터가
`regionName`을 직접 넣는다). 어댑터가 `HeaderArea/StageCard`에 붙어 있고 그 카드가 공용 HUD
프리팹 안이라, **3-86에서 4개 씬이 모두 같은 프리팹을 쓰게 된 덕에 한 번 고치면 전 스테이지에 적용된다.**

**수정 3가지**:
1. **태그 아트를 9분할로 바꿨다.** 알파를 실측하니(147×85) 왼쪽 두 모서리가 사선(x 2~29)이고 오른쪽은
   x 143에서 직선으로 끝나되 우상단만 짧은 사선(x 136~143)이었다. 그래서 `spriteBorder = (34, 0, 12, 0)`
   으로 잡으면 가운데(x 34~135)는 어느 행에서나 직선 테두리뿐이라 **가로로 늘려도 모서리·스티치가
   안 망가진다.** `Background` Image를 `Simple` → `Sliced`로 변경(세로는 안 늘리므로 상하 경계는 0).
2. **어댑터가 이름을 넣을 때마다 태그 폭을 다시 잡는다.** 줄바꿈을 끄고(`enableWordWrapping = false`)
   `preferredWidth`를 재서 `폭 = 글자폭 + 좌여백 + 우여백`. **오른쪽 끝은 Awake에서 기록한 값으로
   고정하고 왼쪽으로만 자라게** 해서 옆 Turn 카드와의 겹침 관계를 시안 그대로 유지한다.
   ⚠️ 줄바꿈을 먼저 끄지 않으면 `preferredWidth`가 "이미 접힌 뒤의 폭"을 돌려줘서 아무리 재도 안 넓어진다.
3. **좌우 여백을 다르게 줬다(좌 13.5 / 우 22).** 시안상 Turn 카드가 이 태그의 오른쪽 끝 15px를 덮고
   있어서(태그 우변 1757 vs Turn 좌변 1742) 좌우를 같게 주면 이름 끝 글자가 Turn 카드 밑으로 들어간다.
   실제로 첫 시도(좌우 13.5)에서 Zone3 이름이 Turn 카드와 1px 겹쳤다. 겹치는 15px + 여유 7px = 22.
   프리팹의 `StageName` 상자 여백도 같은 값으로 맞춰야 계산 폭과 그려지는 위치가 일치한다.

**검증**(4개 씬 각각 Play Mode, 글리프 실측):

| 스테이지 | 이름 | 태그 폭 | 줄수 | 글리프 | Turn 카드와 간격 |
|---|---|---|---|---|---|
| Stage_01 | 북문(외곽) | 147 (최소폭) | 1 | 6/6 | 13px |
| Stage_02 | 상업지구 · 시장가 | 205 | 1 | 8/8 | 7px |
| Stage_03 | 귀족가 · 영주 저택 인근 | 258 | 1 | 10/10 | 7px |
| Stage_04 | 클레르폰 대도시 전역 | 235 | 1 | 9/9 | 9px |

4개 모두 **오른쪽 끝 1757 고정**, 글자가 태그 안에 전부 들어감, Turn 카드 밑으로 안 들어감 ✅.
캡처로 늘어난 태그의 모서리·스티치가 멀쩡한 것도 확인. Console Error/Warning 0.

**앞으로 이름이 더 길어져도** 태그가 따라 늘어난다. 다만 계속 길어지면 왼쪽으로 자라다 화면 밖으로
나가므로, 실질 상한은 `1757 - 화면좌측여백` 정도다(현재 최장 258px, 여유 충분).

**수정한 파일**: `StageRegionNameAdapter.cs`, `PlayHudCanvas_New.prefab`(StageCard 구조/앵커),
`Assets/Belief/UI/PlayHUD/스테이지명 UI.png`(9분할 경계).

---

### 3-91. 메인메뉴 → 4스테이지 전체 자동 플레이 검수 (2026-08-05, 사용자 지시 "처음부터 끝까지 플레이")

**방법**: 실제 플레이어와 같은 입력 경로(카드 선택 → 대상 지정 → **접선 태그 클릭으로 전달 확정**)를
그대로 밟는 임시 자동 플레이 하네스를 만들어 MainMenu에서 시작해 끝까지 돌렸다. `Application.logMessageReceived`를
후킹해 런타임 오류도 같이 수집했다. 검수 후 하네스 파일은 삭제했다(리포지토리에 남기지 않음).

**결과 요약**: 메인메뉴 → Zone1 → Zone2 → Zone3 → Metropolis까지 씬 전환·브리핑·미션 판정·결과 화면이
모두 정상 동작했고, **런타임 Error/Exception 0건**. Stage 1~3은 각 미션이 1턴 만에 클리어됐다.
Stage 4 M01에서 **43 스테이지턴을 돌려도 클리어되지 않아** 거기서 중단했다.

| | 결과 |
|---|---|
| MainMenu → 게임 시작 → Zone1 | 정상 |
| Stage 1 M01/M02 | 각 1턴 클리어, 결과 화면 데이터(제목/설명/NO./STAGE/구역/턴) 정상 |
| Stage 2 M01/M02 | 각 1턴 클리어 |
| Stage 3 M01/M02 | 각 1턴 클리어 |
| Stage 4 M01 | **미클리어** (아래 참조) |
| 런타임 오류 | **0건** |

**★ 문제 1 (구조적) — Stage_04 M01은 사실상 달성하기 어렵다.** 조건을 데이터까지 따라가 보면:
- 조건 A(`TarnishedRumorReachesLord`)가 보는 카드 `C-POL-01`/`C-PUB-01`/`C-REL-01`은 **셋 다 확산형(Spread)**
  이다. 즉 **영주에게 직접 전달할 수 없고**, 소문이 영주가 있는 곳까지 퍼져 영주가 스스로 판단해야만 켜진다.
- 그 3장은 30장짜리 `CardPool_Default`에 **각 1장씩**만 있다(뽑힐 확률이 낮다).
- 조건 B(`TwoDistrictsRumor`)는 **저택가와 시장 두 곳에 동시에 활성 소문**을 요구한다 = 확산 2번을 따로 써야 한다.
- 그런데 `Mission_Stage04_01.turnLimit = 4`다. 4턴 안에 "특정 3장 중 하나를 뽑아 영주까지 닿게 하기 +
  서로 다른 두 구역에 소문 유지"를 모두 만족해야 한다.

Stage 1~3이 1턴에 끝나는 것과 비교하면 난이도 절벽이 지나치게 크다. 손볼 방향은
(a) `turnLimit`을 늘리거나, (b) 조건 A를 이미 완화된 다른 미션들처럼 "영주가 소문을 주시/조사"
(WorldState 기반)로 바꾸거나, (c) 조건 B를 한 구역만으로 낮추는 것. **밸런스 결정이라 임의로 바꾸지 않았다.**

**★ 문제 2 — 스테이지 진행 턴 표시가 상한을 넘어 계속 올라간다.** 실측에서 `S13/12`를 지나 `S43/12`까지
갔다. `TurnSystem.StageTurn`/`StageMaxTurns`는 코드 주석대로 **UI 표시 전용이고 판정에 관여하지 않는 것이
의도**지만, 표시가 자기 최대값을 넘겨 계속 커지는 건 플레이어에게 그대로 보인다. 최소한 표시를 상한에서
멈추거나, 스테이지 턴 제한을 실제로 강제할지 결정이 필요하다.

**★ 문제 3 — 스테이지의 마지막 미션을 깬 직후 "미션 없음" 상태에서도 카드를 낼 수 있다.**
로그: `Stage_01 T1/4 S1/8 | 미션없음 | 손패 3 | 확산 "민원 처리 지연"->여관`. 씬 전환 직전의 짧은
창이지만 그 사이 입력이 그대로 먹혀 카드 한 장이 소모된다.

**★ 관찰 — 브리핑 "작전 실행" 버튼이 연출(페이드) 동안 계속 눌린다.** 하네스가 1초 남짓 동안 같은
버튼을 55~75회 눌렀다. 사람 손으로는 잘 안 나타나지만 연타 시 중복 입력이 막히지 않는다.

**부수 수정**: 3-90에서 쓴 `TMP_Text.enableWordWrapping`이 deprecated 경고를 내서
`textWrappingMode = TextWrappingModes.NoWrap`으로 교체했다. 현재 Console 경고 0.

---

### 3-92. 어느 스테이지에도 안 쓰이는 구버전 장소 애셋 7개 삭제 (2026-08-05, 사용자 지시)

3-87에서 확인한 "사진 없는 장소 8개" 중 **실제로 스테이지에 들어가는 건 저택가 하나**뿐이고
나머지 7개(`Loc_castle_gate` 성문 / `Loc_courthouse` 재판소 / `Loc_customs` 세관 / `Loc_docks` 부두 /
`Loc_fish_market` 어시장 / `Loc_tavern` 선술집 / `Loc_temple` 신전)는 어느 `StageData.locations`
에도, 접선 지점에도, 미션 조건에도 없는 구버전 잔재였다.

**삭제 전 참조를 전수 조사했더니 살아있는 애셋 중 딱 하나가 이들을 참조하고 있었다** —
`Loc_manor_row.connectedLocations`가 `castle_gate`/`temple`/`courthouse`를 물고 있었다. 그냥 지우면
Missing 참조가 남으므로 **먼저 그 3개 항목을 배열에서 제거한 뒤** 애셋을 삭제했다.
(나머지 상호 참조는 잔재끼리라 함께 사라진다.)

⚠️ 그 결과 **`Loc_manor_row.connectedLocations`가 0개가 됐다.** 원래 물고 있던 3곳이 전부 존재하지
않는 장소였으니 소문 전파상 실제 동작은 그대로지만(전파는 `installer.Locations`에 있는 장소만 대상),
저택가를 다른 실제 장소와 연결할지는 기획 판단이 필요하다. Stage_04 M01의 조건 B는 플레이어가
저택가에 **직접 확산**해야 켜지는 것이라 지금도 달성 경로는 막히지 않는다.

**검증**: 남은 `LocationData` **16개**, 끊긴 참조 **0개**, Stage_01~04의 `locations` 배열에 null **0개**,
사진 없는 장소는 이제 **저택가 하나뿐**. Console Error 0.

---

### 3-93. 저택가 제거(장소 14개로 정리) + Metropolis 표시 크기 확대 (2026-08-05, 사용자 지시)

**장소 수가 기획과 안 맞았다.** 기획상 장소는 **14개**(1스테이지 4 + 2스테이지 4 + 3스테이지 5 +
Metropolis 신규 1 = 알현실 앞 광장)여야 하는데 16개였다. 초과분은 접선 전용 `뒷골목`(장소가 아니라
전달 시스템 자리라 별도)과 **`저택가`(기획에 없는 장소)** 둘이었다.

**저택가는 그냥 지울 수 없었다** — `Condition_Stage04_M01_RumorAtNobleDistrict.targetLocation`이
저택가였다. Stage 4 M01의 "귀족가와 상업지구 양쪽에서 동시에 소문" 중 **귀족가 쪽 판정 지점**이라
그냥 지우면 M01이 영구히 클리어 불가가 된다. 그래서 사용자 결정에 따라 **대저택 응접실**
(`Loc_MANOR_RECEPTION`, 3스테이지 귀족가의 중심 장소이자 상업지구 짝인 `시장`과 위상이 맞음)으로
대상을 옮기고, 조건 문구도 `"저택가에 소문이 퍼져 있음"` → `"대저택 응접실에 소문이 퍼져 있음"`으로 갱신했다.

지운 순서(참조를 남기지 않기 위해 역순으로):
`조건 targetLocation 교체` → `Stage_04.locations / locationLayout에서 제거` →
`Metropolis.unity의 GameInstaller.allLocations(폴백 배열)에서 제거` → `애셋 삭제`.
마지막 단계 전에 남은 참조를 전수 재조사해 씬 하나만 남은 것을 확인하고 처리했다.

**결과**: `LocationData` **15개** = 기획상 장소 **14개** + 접선 전용 뒷골목 1. 사진 없는 장소 **0개**
(저택가가 사라지면서 3-87의 "사진 없는 카드" 문제도 자연히 해소).

---

**Metropolis만 유독 작아 보이던 문제**도 같이 처리했다. 카메라 `orthographicSize`가 Zone1 5 / Zone2·3 6 /
Metropolis 14라 **같은 월드 크기라도 화면상 최대 2.8배까지 차이**가 난다(1 유닛 = 108px vs 38.6px).

`StageData.worldViewScale`(기본 1)을 추가하고 `WorldPresenter`가 장소 카드·NPC 인스턴스에 적용한다.
**핵심은 배치 좌표를 건드리지 않고 보이는 크기만 키우는 것** — 대신 `ComputeNpcSlot`의 좌우 슬롯
간격과 세로 오프셋도 같은 배율로 곱해야 카드만 커지고 NPC가 카드 안으로 파고드는 일이 없다.

**배율은 계산이 아니라 실측으로 정했다.** 좌표만으로 계산한 보수적 한계는 1.32였지만(모든 장소가
NPC를 양옆 최대로 펼쳤다고 가정), 실제 시작 배치에서는 그만큼 펼쳐지지 않아 여유가 있었다:

| 배율 | 카드 화면 크기 | 카드 겹침 | 타 장소 NPC 겹침 | 화면 밖 |
|---|---|---|---|---|
| 1.0 (이전) | 35×56px | 0 | 0 | 0 |
| 1.5 | 52×85px | 0 | 0 | 0 |
| **1.8 (채택)** | **63×102px** | **0** | **0** | **0** |
| 참고: Zone2·3 | 81×131px | — | — | — |

Stage_01~03은 `worldViewScale = 1`로 두어 **기존 화면이 전혀 바뀌지 않는다**(Metropolis만 1.8).

⚠️ **알려진 한계**: 위 검증은 **시작 배치** 기준이다. 게임 중 NPC가 한 장소로 몰리면(예: M03의
"광장에 3명 집합") 그 장소의 가로 폭이 커져 이웃과 겹칠 수 있다. 좌표상 가장 가까운 쌍
(영주 저택 앞 ↔ 경비 초소, dx=3.65)은 **배율 1.0에서도 3명이 모이면 이미 아슬아슬**하므로 이는
확대로 새로 생긴 문제가 아니라 원래 배치의 여유 부족이 배율만큼 드러나는 것이다. 더 여유를 두려면
1.5로 낮추거나 그 두 장소 좌표를 벌리면 된다.

**수정한 파일**: `StageData.cs`(`worldViewScale` 추가), `WorldPresenter.cs`(적용 + NPC 슬롯 간격 보정),
`Stage_04.asset`, `Condition_Stage04_M01_RumorAtNobleDistrict.asset`, `Metropolis.unity`,
`Loc_manor_row.asset`(삭제).
